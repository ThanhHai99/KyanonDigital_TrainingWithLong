...
export class SaleService {
  ...
  async create(
    ...
    if (isSaleExists) throw new HttpException('The sale code already in use', HttpStatus.CONFLICT)

    // Validate item length vs amount length
    if (dataCreateSaleItem.item_id.length < 1 || !isArraysSameLength(dataCreateSaleItem.item_id, dataCreateSaleItem.amount))
    ...
    await transactionEntityManager
      ...
      .then(async (resolve) => {
        ...
        await this.saleItemService.create(transactionEntityManager, dataCreateSaleItem)
      ...
      .catch((reject) => {
        throw new HttpException('The sale cannot create', HttpStatus.INTERNAL_SERVER_ERROR)
      })
  }

  async update(transactionEntityManager: EntityManager, id: number, data: BodyUpdateSale): Promise<any> {
    const sale = await this.saleRepository.findOne(id)
    if (!sale) throw new HttpException('Sale is not found', HttpStatus.NOT_FOUND)

    if (sale.applied) throw new HttpException('Sale is applied cannot update', HttpStatus.OK)

    if (data.code) {
      const isCodeExists = await this.findByCode(data.code)
      if (isCodeExists) throw new HttpException('The code already in use', HttpStatus.CONFLICT)
    }

    // Update a sale
    await transactionEntityManager.update(Sale, id, data).then(async (resolve) => {
  ...
  async totalDecreaseCostByCode(transactionEntityManager: EntityManager, itemInOrder: ItemOrder[], code: string): Promise<number> {
    ...
    for (let i = 0; i < itemInOrder.length; i++) {
      ...
      const isItemStillOnSale = await this.saleItemService.isItemStillOnSale(sale.id, <number>itemId)
      if (isItemStillOnSale) {
        const item = await this.itemService.findById(<number>itemId)
        total += ((item.price * sale.discount) / 100) * amount
        await this.saleItemService.updateAmount(transactionEntityManager, sale.id, <number>itemId, amount)
      }
    }
