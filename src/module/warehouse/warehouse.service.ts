...
export class WarehouseService {
  ...
  async getInventory(): Promise<Warehouse[]> {
    return await this.warehouseRepository.find({
      expiration_date: Raw((alias) => `${alias} < DATE_ADD(DATE(NOW()), INTERVAL 1 MONTH)`)
    })
  }

  async create(transactionEntityManager: EntityManager, data: BodyImporting): Promise<any> {
    // Check data is valid
    if (data.item_id.length < 0 || !isArraysSameLength(data.item_id, data.amount, data.expiration_date, data.price)) {
    ...
    for (let i = 0; i < data.item_id.length; i++) {
      await transactionEntityManager
        ...
        .catch((reject) => {
          throw new HttpException('The warehouse cannot create', HttpStatus.INTERNAL_SERVER_ERROR)
        })
    }
  }
  ...
  async isEnoughQuantityToExport(itemInOrder: ItemOrder[]): Promise<boolean> {
    ...
    for (let i = 0; i < itemInOrder.length; i++) {
      ...
      if (currentQuantityOfItem[0].sumAmount < amount) throw new HttpException('Item quantity is not enough', HttpStatus.OK)
    }
    return true
  }

  async exportProduct(transactionEntityManager: EntityManager, itemInOrder: ItemOrder[], userId: number): Promise<any> {
    for (let i = 0; i < itemInOrder.length; i++) {
      const { item, amount } = itemInOrder[i]
      let _item: Item = <Item>item
      const { id: itemId } = _item
      ...
      let warehouse = await this.warehouseRepository.find({
        where: {
          item: itemId,
          amount: MoreThan(0),
          expiration_date: Raw((e) => `${e} > DATE_ADD(DATE(NOW()), INTERVAL 1 MONTH)`)
        },
      ...
      for (let j = 0; j < warehouse.length; j++) {
        ...
        warehouse[j].amount = warehouse[j].amount <= _amount ? 0 : warehouse[j].amount - _amount
