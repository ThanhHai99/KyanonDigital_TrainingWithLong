...
export class WarehouseController {
  ...
  @Get()
  async getAll(@Res() res): Promise<ResponseGetWarehouse> {
    try {
      const warehouses: Warehouse[] = await this.warehouseService.getAll();
  ...
  @Get('inventory')
  async inventory(@Res() res): Promise<ResponseGetWarehouse> {
    try {
      const subtractOneMonth = moment()
        .subtract(1, 'months')
        .format('YYYY-MM-DD');
      const warehouses: Warehouse[] = await this.warehouseService.getInventory(
