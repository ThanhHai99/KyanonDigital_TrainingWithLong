...
export class ItemLogService {
  ...
  async create(transactionEntityManager: EntityManager, data: BodyCreateItemLog): Promise<any> {
    await transactionEntityManager.insert(ItemLog, data).catch((reject) => {
      throw new HttpException('The item log cannot create', HttpStatus.INTERNAL_SERVER_ERROR)
    })
  }
}
