...
export class UserService {
  ...
  async createEmployee(data: BodyCreateUser): Promise<any> {
    ...
    if (!thisRole || thisRole.id === 1) throw new HttpException('Role is incorrect', HttpStatus.NOT_FOUND)
  ...
  async update(id: number, data: Partial<BodyUpdateUser>): Promise<any> {
    ...
    if (data.role) {
      const thisRole = await this.roleService.findById(data.role)
      if (!thisRole || id === 1) throw new HttpException('Role is incorrect', HttpStatus.NOT_FOUND)
    }
