...
export class PriceLogService {
  ...
  async create(transactionEntityManager: EntityManager, data: BodyCreatePriceLog): Promise<any> {
    await transactionEntityManager.insert(PriceLog, data).catch((reject) => {
      throw new HttpException('The price log cannot create', HttpStatus.INTERNAL_SERVER_ERROR)
    })
  }
}
